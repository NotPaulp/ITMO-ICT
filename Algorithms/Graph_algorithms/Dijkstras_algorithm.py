import collections
Roads = {'Alabama':{'Georgia':263.4 , 'Tennessee':255.3, 'Mississippi':157.6, 'Florida':547.0 },
	'Alaska':{},
...
first_state=input('Enter the starting state: ')
distance={}
if not(first_state in Roads):
	print("No such state")
elif first_state=='Alaska' or first_state=='Hawaii':
	print(f"There are no direct roads from {first_state} to any other state")
else:
	queue=collections.deque()
	distance[first_state]=0
	queue.append(first_state)
	while queue:
		state=queue.pop()
		for connected_state in Roads[state]:
			if connected_state not in distance or distance[state]+Roads[state][connected_state]<distance[connected_state]:
				distance[connected_state]=distance[state]+Roads[state][connected_state]
				queue.append(connected_state)
	print(f"The state           Shortest path to it from {first_state}")
	for state in distance:
		print(state+" "*(20-len(state))+str(distance[state]))
	state='Alaska'
	print(state + " " * (20 - len(state)) + "NA")
	state="Hawaii"
	print(state + " " * (20 - len(state)) + "NA")
